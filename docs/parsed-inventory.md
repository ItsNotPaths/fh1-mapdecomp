...
| `Colorado_00.pvs` | 🟡 | (probe-level only) | Magic `FPVS` v0x32, 3.2 MB. Per-zone visible-zones list. Header offsets table identified, body block layout undecoded. |
| `PVSZLookup_00.dat` | 🟡 | (probe-level only) | 1,434 × `(zone_hash, zone_index)` pairs. Layout fully understood; **no first-class parser yet**. |
| `__R00Z*.pvsz` (1,434 files inside `bin.zip`) | ✅ | (probe-level) | Per-zone streaming manifests: `u32 BE count, count × u32 BE (0x80000000 \| rmb_handle)`. Decoded across all 1,434 zones as a `handle -> zone_set` reverse index covering 59,599 unique handles (96.8 % of .col handles). 2026-04-27 (fourth pass) used this + HEXY zone↔cell map to derive landmark placements (output: `probes/out/pvsz_handle_to_zones.json`). Probe: `probe_pvsz_zone_xref.py`. |
...
|---|---:|---|---|---|
| `*.pgeo` | 41,587 entries (7,970 case-unique) | 🟡 | `pgeo.py`, `pgeo_body/*` | Header + variant classifier ✅. Body decoders: **terrain ✅, v42k7 ✅, v42k7 proc-subvariant ✅**. Undecoded body classes: **grass (11,292), crowd (8,544), vegetation (3,316), v44k5 (1,578), landmark_anim (447)** — spectators / grass-tufts / light-glows / festival-rides / animated-procs. The landmark_anim PGEOs carry embedded `Anim_ANIM_*` strings naming the rig (Ferris, Zipper, Windmill, autoshow lights, etc.); body format undecoded. |
...
| `*.pvsz` | 3,502 | 🟡 | (probe only) | Per-zone streaming manifest, fully understood; no first-class parser. |
