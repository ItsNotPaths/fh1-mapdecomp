...
|---|---:|---|---|---|
| `*.pgeo` | 41,587 entries (7,970 case-unique) | 🟡 | `pgeo.py`, `pgeo_body/*`, `crowd_inst.py`, `grass_inst.py` | Header + variant classifier ✅ (variants confirmed against xex RTTI `proceduralGeometry::CProcedural*` — see `pgeo-body.md §0`). Body decoders: **terrain ✅, v42k7 ✅, v42k7 proc-subvariant ✅ (trees; positions decoded but extractor unwired), crowd ✅ (inanimate-only, see §3.6), grass ✅ (roadside/festival scatter; trees NOT in this variant — see §3.4), light_glows 🟡 (body shape decoded — §3.5 — extractor unwired)**. Grass body parses 11,292/11,292 files; 1.05M placement positions decoded (287k unique after chunk-pool dedup); visually verified 2026-05-03 — covers roadside grass only; trees live in v42k7 proc-inline `models_proc_clrd_trees_*`, not yet wired as a placement extractor. Per-instance template→rmb-handle lookup is external to the grass PGEO; ships a synthetic bright-yellow cylinder placeholder. Crowd body parses 8,514/8,525 files; 11 outliers are Format C and deferred. Undecoded body classes: **v44k5 (1,578; FX emitters), landmark_anim (447; festival rides + autoshow rigs)**. The historical "vegetation" variant (3,316 files) was renamed `light_glows` after the `Glow_*` descriptors + RTTI `CProceduralLightGlows` confirmed it carries light-entity data, not foliage. |
...
| `*.fiz` | 16,934 | 🟡 | (probe-level) | **Confirmed foliage with world-coord header.** Magic `'fiz '` @+0x00, version `1` @+0x04. Header carries count fields, then 4×f32 BE world bbox @+0x38 (e.g. `(-2916.86, -2796.5, -2348.80, -2225.5)` — in Colorado bounds), cell size 10 m, quant step 0.1 (`0x3DCCCCCD`). Filenames are zone-numbered (`1024.fiz`..`16934.fiz` etc.) — one file per zone. Probable per-zone foliage density list. **Investigation dead-ended 2026-04-29** — Block-A probe ruled out triangle topology; not a placement source. See memory `project_fh1_fiz_format_2026-04-29`. Probe: `probe_bix_fiz.py`. |
...
|---|---|---|
| `out/terrain_hi/` | `rmb.py` terrain prefixes (TERR_/Plains_/Mountains_/Foothills_/RedRock_/Reservoir_/Blends_/CLRD_/ROAD_/road_; `rmb.TERRAIN_TAG_PREFIXES`) | ✅ 1,047 unique LOD00 tiles (2.54M verts) including non-TERR mountain/plateau/redrock zones missed by the old TERR-only filter — +720 net-new tiles 2026-05-03 |
...
7. **`.fiz` foliage parser** — investigation dead-ended 2026-04-29 (see memory `project_fh1_fiz_format_2026-04-29`). Block-A header is decoded but Block-B trailer is a render-flag bitmask, not topology; the file is not a placement source. Don't reopen without new external evidence.

8. **PGEO body decoders for grass / light_glows / v44k5 / landmark_anim.** Crowd is decoded (see row above). The remainder hold: grass tufts (engine class `CProceduralVegetation` — actual foliage), light entity data (`CProceduralLightGlows` — body shape decoded, extractor unwired), animated FX procs, festival rides respectively. None of these carry placement tables (already verified) but each holds per-instance data we currently render as bbox-placeholder cubes.
