...
def parse_args():
    ...
    ap.add_argument("--crowd-inst", default="",
                    help="directory with inanimate-crowd PGEO instance per-blob "
                         ...
                         "grandstands)")
    ap.add_argument("--grass-inst", default="",
                    help="directory with `grass` PGEO foliage-scatter "
                         "instance per-blob .npz + index.json "
                         "(CProceduralVegetation; placeholder Grass_LOD01 mesh)")
    ...
    ap.add_argument("--keep-bbox-cubes", action="store_true",
                    help="emit bbox cubes for grass/light_glows/crowd/landmark_anim "
...
VARIANT_COLORS = {
    ...
    "light_glows":   (0.15, 0.55, 0.45, 1.0),
    "crowd":         (0.85, 0.25, 0.25, 1.0),
    "crowd_inst":    (0.95, 0.45, 0.20, 1.0),
    # Bright yellow — temporary marker color so cylinder placeholders
    # are obvious in the scene while the per-instance template→handle
    # mapping is still unresolved.
    "grass_inst":    (1.00, 0.95, 0.00, 1.0),
...
def main():
    ...
    BBOX_NOISE_VARIANTS = (
        ...
        "grass", "light_glows",      # PGEO bodies undecoded — placeholder
                                     # for future grass / light-glow extractors
    ...
    if args.grass_inst:
        _import_inst_dir(Path(args.grass_inst),
                         label="grass_inst",
                         collection="fh1_grass_inst",
                         material_variant="grass_inst",
                         src_prefix="grass_src",
                         inst_prefix="grass_inst",
                         blob_prefix="grass_blob")
